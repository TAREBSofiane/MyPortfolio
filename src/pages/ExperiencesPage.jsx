...
import { ExperienceModalContent } from '../components/ModalContents.jsx';

export default function ExperiencesPage() {
  const { profile, experiences, status } = useData();
  const [selected, setSelected] = useState(null);
  const pageMeta = profile?.pages?.experiences;

  usePageMeta(
    `${pageMeta?.title || 'Mes Experiences'} | ${profile?.identity?.name || ''}`,
    pageMeta?.subtitle
  );
    ...
    <>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {status === 'loading'
              ...
              : experiences.map((exp, index) => (
                  <ExperienceCard key={exp.title} exp={exp} index={index} onOpen={setSelected} />
                ))}
          </div>
        </div>
      </section>
      ...
      {selected && (
        <DetailModal title={selected.title} onClose={() => setSelected(null)}>
          <ExperienceModalContent exp={selected} />
        </DetailModal>
      )}
    </>
  );
