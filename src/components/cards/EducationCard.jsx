...
export default function EducationCard({ edu, index, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        <h4 className="text-lg font-medium text-primary-600 dark:text-primary-400 mb-3">{edu.institution}</h4>
        <div className="text-gray-600 dark:text-gray-300 mb-4 line-clamp-3">{edu.formation}</div>

        <div className="mb-4">
          <h5 className="text-sm font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">
            {t('card.skillsAcquired')}
          </h5>
          <div className="flex flex-wrap gap-2">
            ...
            {edu.skills.length > 3 && (
              <span className="px-2 py-1 bg-gray-100 dark:bg-gray-600 text-xs rounded">
                +{edu.skills.length - 3}
              </span>
            )}
          </div>
        ...
        <button type="button" onClick={() => onOpen(edu)} className="btn-voir-plus">
